...
class ClassicDeriv:
  ...
  def __init__(self, n_reactions = 5000, n_species = 2000):
    ...
    self.number_of_prodcuts = np.zeros((self.n_reactions), dtype=np.int64)
  ...
  def randomize(self):
    ...
    self.number_of_prodcuts = np.random.randint(0, np.iinfo(np.int64).max, self.n_reactions) % 10 + 1
    ...
    for rxn_idx in range(self.n_reactions):
      ...
      for product_idx in range(self.number_of_prodcuts[rxn_idx]):
  ...
  def apply(self, parent):
    ...
    self.number_of_prodcuts = np.array(parent.number_of_prodcuts)
